...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">{{ trans('dashboard/terms.show_all_paymentTerms_in_sidebar') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0"></span>
            </div>
        ...
        <div class="d-flex my-xl-auto right-content">
            <div class="pr-1 mb-3 mb-xl-0">
                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#add">
                    {{ trans('dashboard/terms.add_new_paymentTerm') }}
                </button>
            </div>
        </div>
    </div>
...
@section('content')
                ...
                <div class="card mg-b-20">
                    <div class="card-header pb-0">
                        <div class="d-flex justify-content-between">
                            <h4 class="card-title mg-b-0">{{ trans('dashboard/terms.show_all_paymentTerms_in_sidebar') }}</h4>
                        </div>
                            ...
                            <table id="example" class="table key-buttons text-md-nowrap">
                                ...
                                @foreach($paymentTerms as $paymentTerm)
                                    <tr>
                                        ...
                                        <td>{!! \Str::limit($paymentTerm->name, 35) !!}</td>
                                        <td>{{ $paymentTerm->created_by }}</td>
                                        <td>{{ $paymentTerm->updated_by }}</td>
                                        <td>{{ $paymentTerm->created_at->diffForHumans() }}</td>
                                        <td>
                                            <a class="modal-effect btn btn-sm btn-info" data-effect="effect-scale" data-toggle="modal" href="#edit{{$paymentTerm->id}}">
                                                <i class="las la-pen"></i>
                                            </a>
                                            <a class="modal-effect btn btn-sm btn-danger" data-effect="effect-scale" data-toggle="modal" href="#delete{{$paymentTerm->id}}">
                                                <i class="las la-trash"></i>
                                            </a>
                                        </td>
                                    </tr>
                                    @include('Dashboard.Terms.PaymentTerms.btn.edit')
                                    @include('Dashboard.Terms.PaymentTerms.btn.delete')
                                @endforeach()
                                </tbody>
                            </table>
                        </table>
        ...
        @include('Dashboard.Terms.PaymentTerms.btn.add')
    </div>
    <!-- Container closed -->
</div>
<!-- main-content closed -->
