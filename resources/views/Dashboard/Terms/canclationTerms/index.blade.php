...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">{{ trans('dashboard/terms.show_all_canclationTerms_in_sidebar') }}</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0"></span>
            </div>
        ...
        <div class="d-flex my-xl-auto right-content">
            <div class="pr-1 mb-3 mb-xl-0">
                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#add">
                    {{ trans('dashboard/terms.add_new_canclationTerm') }}
                </button>
            </div>
        </div>
    </div>
...
@section('content')
                ...
                <div class="card mg-b-20">
                    <div class="card-header pb-0">
                        <div class="d-flex justify-content-between">
                            <h4 class="card-title mg-b-0">{{ trans('dashboard/terms.show_all_canclationTerms_in_sidebar') }}</h4>
                        </div>
                    ...
                    <div class="card-body">
                        <table class="table-responsive">
                            <table id="example" class="table key-buttons text-md-nowrap">
                                ...
                                <tbody>
                                @foreach($cancelationTerms as $cancelationTerm)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{!! \Str::limit($cancelationTerm->name, 35) !!}</td>
                                        <td>{{ $cancelationTerm->created_by }}</td>
                                        <td>{{ $cancelationTerm->updated_by }}</td>
                                        <td>{{ $cancelationTerm->created_at->diffForHumans() }}</td>
                                        <td>
                                            <a class="modal-effect btn btn-sm btn-info" data-effect="effect-scale" data-toggle="modal" href="#edit{{$cancelationTerm->id}}">
                                                <i class="las la-pen"></i>
                                            </a>
                                            <a class="modal-effect btn btn-sm btn-danger" data-effect="effect-scale" data-toggle="modal" href="#delete{{$cancelationTerm->id}}">
                                                <i class="las la-trash"></i>
                                            </a>
                                        </td>
                                    </tr>
                                    @include('Dashboard.Terms.canclationTerms.btn.edit')
                                    @include('Dashboard.Terms.canclationTerms.btn.delete')
                                @endforeach()
                                </tbody>
                            </table>
                        </table>
        ...
        @include('Dashboard.Terms.canclationTerms.btn.add')
    </div>
    <!-- Container closed -->
</div>
<!-- main-content closed -->
